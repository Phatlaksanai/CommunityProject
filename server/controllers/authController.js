...
exports.register = async (req, res) => {
  try {
    ...
    if (username.length < 3 || username.length > 35) {
      return res
        .status(400)
        .json({ error: "Username must be between 3 and 35 characters" });
    }
    if (password.length < 10 || password.length > 20) {
      return res
        .status(400)
        .json({ error: "Password must be between 10 and 20 characters" });
    }
...
exports.sendOtpRegister = async (req, res) => {
  try {
    ...
    const { error: otpError } = await db.from("otps").upsert(
      {
      ...
      { onConflict: "user_id" },
    ); // ป้องกันการสร้าง OTP ซ้ำซ้อนให้ user คนเดิม
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (displayName.length < 3 || displayName.length > 35) {
      return res
        .status(400)
        .json({ error: "Display name must be between 3 and 35 characters" });
    }
    ...
    if (description.length < 3 || description.length > 500) {
      return res
        .status(400)
        .json({ error: "Description must be between 3 and 500 characters" });
    }
...
exports.deleteAccount = async (req, res) => {
  ...
  if (!email || !otp || !userId) {
    return res
      .status(400)
      .json({ error: "Email, OTP, and User ID are required" });
  }
  ...
  try {
    ...
    if (otpError || !otpData) {
      return res
        .status(400)
        .json({ error: "Invalid OTP request or OTP not found" });
    }
...
exports.getUserById = async (req, res) => {
  ...
  try {
    const { data: user, error } = await db
      .from("users")
      .select("user_id, username, name, profilePic, balance")
      .eq("user_id", userId)
      .single(); // ใช้ .single() เพื่อให้ได้ object เดียวแทนที่จะเป็น array
